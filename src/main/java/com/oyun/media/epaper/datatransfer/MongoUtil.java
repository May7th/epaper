...
public enum MongoUtil {
    ...
    static {
        ...
        String ip = "202.207.12.187";
        int port =90;
        MongoCredential credential = MongoCredential.createCredential("epaper","dno", "aoyunMongo832".toCharArray());
        MongoCredential pcredential = MongoCredential.createCredential("epaper","picsdb", "aoyunMongo832".toCharArray());
        //instance.mongoClient = new MongoClient(ip, port);
        instance.mongoClient = new MongoClient(new ServerAddress(ip, port), Arrays.asList(credential, pcredential), MongoClientOptions.builder().build());
        // 大部分用户使用mongodb都在安全内网下，但如果将mongodb设为安全验证模式，就需要在客户端提供用户名和密码：
        //boolean auth = db.authenticate("epaper", "aoyunmongo832");
    ...
    public DB getDBDB(String dbName) {
        if (dbName != null && !"".equals(dbName)) {
     ...
     * @throws IOException 
     */
    public void savePic(DB db, String newFN, String oriFN) throws IOException{
    	if(newFN == null || newFN.trim().length() <= 0){
    		return;
    	}
    	if(oriFN == null || oriFN.trim().length() <= 0){
    		return;
    	}
    	GridFS gfsPhoto = new GridFS(db, newFN);
		File imageFile = new File(oriFN);
		GridFSInputFile gfsFile = gfsPhoto.createFile(imageFile);
		gfsFile.setFilename(newFN);
		gfsFile.save();
    }
