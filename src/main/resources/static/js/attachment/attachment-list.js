...
var table = $('#data-table').DataTable({
    ...
    columnDefs: [{ // 定义表格样式
        ...
        targets: 2,
        render: function (data, type, row, meta) {
            return '<td><img style="height: 100px" title="查看"' +
    ...
    ajax: {
        type: "GET",
        url: "/attachments", // 服务器url
        cache: false,
        data: function (data) {
            var postData = {};
            houseStatus = $('#attachmentStatus').val(),
            ...
            if (oldName.length > 0) {
                postData.oldName = oldName;
            }

            postData.temp = houseStatus;
...
$('#attachmentSearch').on('click', null, function () {
    reloadTable();
});
